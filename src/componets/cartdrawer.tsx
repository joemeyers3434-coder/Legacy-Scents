        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem 2rem' }}>
          {items.length === 0 ? (
            <div style={{ textAlign: 'center', paddingTop: '4rem' }}>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.9rem', marginBottom: '2rem' }}>Your cart is empty</p>
            </div>
          ) : (
            items.map(item => (
              <div key={item.id} style={{ display: 'flex', gap: '1rem', paddingBottom: '1.5rem', marginBottom: '1.5rem', borderBottom: '0.5px solid rgba(201,168,76,0.1)' }}>
                <div style={{ width: '70px', height: '90px', background: '#111111', border: '0.5px solid rgba(201,168,76,0.2)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {items.length > 0 && (
          <div style={{ padding: '1.5rem 2rem', borderTop: '0.5px solid rgba(201,168,76,0.15)' }}>
            ...
            <button
              ...
              style={{ width: '100%', padding: '1rem', background: '#C9A84C', color: '#0A0A0A', border: 'none', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', cursor: 'pointer' }}>
              Checkout
            </button>
          </div>
        )}
